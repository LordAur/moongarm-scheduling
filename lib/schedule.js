    ...
    if (this.postUrl !== null) {
      cron.schedule(this.schedule, () => {
        request
          .post(this.postUrl)
          .on('response', () => {
            callback(`Running rest api from ${this.postUrl} every minutes success.`);
          })
          .on('error', (error) => {
            callback(`Running rest api from ${this.postUrl} every minutes failed with error message: ${error}`);
          });
      }, {
        scheduled: true,
        timezone: this.timezone,
    ...
    if (this.putUrl !== null) {
      cron.schedule(this.schedule, () => {
        request
          .put(this.putUrl)
          .on('response', () => {
            callback(`Running rest api from ${this.putUrl} every minutes success.`);
          })
          .on('error', (error) => {
            callback(`Running rest api from ${this.putUrl} every minutes failed with error message: ${error}`);
          });
      }, {
        scheduled: true,
        timezone: this.timezone,
